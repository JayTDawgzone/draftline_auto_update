...
base_url = 'https://www.draftlinesmartsystem.com/ReccuringTrips.aspx'
df = pd.read_excel('account_list.xlsx')
for index, row in df.iterrows():

    # tripID = str(round(row['TripID']))
    account = row['Location Name']
    # title = row['Rte']
    ...
    driver.find_element_by_id('addNew_tbTitleNew').send_keys('00 - Line Cleaning')
    driver.find_element_by_id('addNew_tbAccountNew').send_keys(account)
    sleep(4)
    driver.find_element_by_link_text(account).click()
    sleep(1)
    driver.execute_script("document.getElementById('addNew_tbFromDateNew').value = " + "'" + date + "'" + ";");
